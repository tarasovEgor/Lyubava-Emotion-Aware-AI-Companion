...
class DummyLLMClient:
    ...
    def generate_reply(
        self, user_message: str, system_prompt: str, history_messages=None
    ):
        _ = history_messages
        return type(
            "DummyReply",
...
def client_with_chat(monkeypatch) -> Generator[TestClient, None, None]:
    monkeypatch.setenv("MODEL_DIR", "models/definitely-missing-ci-model")
    monkeypatch.setattr(
...
def test_get_admin_predictions_returns_items(client_with_chat):
    chat_payload = {
        "session_id": "admin-feed-session",
        "message": "Мне немного тревожно",
    }
